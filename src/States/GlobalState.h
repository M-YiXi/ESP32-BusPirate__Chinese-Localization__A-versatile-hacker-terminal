...
private:
    ...
    // WiFi AP 凭证
    std::string apName = "ESP32-总线-海盗"; // 汉化
    std::string apPassword = "已准备好上车"; // 汉化
    ...
    bool threeWireOrg8 = false; // 默认 16 位组织
    uint8_t threeWireeEepromModelIndex = 0; // 默认 EEPROM 型号索引

    // UART 默认配置
    unsigned long uartBaudRate = 9600;
    uint32_t uartConfig = 0x800001c; // 默认 SERIAL_8N1
