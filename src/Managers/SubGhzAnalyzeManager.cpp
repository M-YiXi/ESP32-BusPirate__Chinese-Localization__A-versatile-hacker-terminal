...
std::string SubGhzAnalyzeManager::analyzeFrame(const std::vector<rmt_item32_t>& items, float tickPerUs) {
    ...
    if (items.empty()) { r.notes = "空帧"; return formatFrame(r); }
    ...
    if (r.encoding == RfEncoding::PulseLength) {
        ...
        if (decodePT2262Like(T, items, tickPerUs, hex, bits)) {
            ...
            if (bits >= 20 && bits <= 36 && ratio > 2.f) {
                r.protocolGuess = "EV1527/PT2262类遥控";
            } else {
                r.protocolGuess = "通用脉冲长度编码";
            }
            r.confidence = clamp01(0.6f + 0.2f * (ratio > 2.5f));
        } else {
            r.protocolGuess = "脉冲长度编码(未解码)";
            r.confidence = 0.35f;
        }
    } else if (r.encoding == RfEncoding::Manchester) {
        r.protocolGuess = "通用曼彻斯特编码";
        r.confidence = 0.5f;
    } else if (r.encoding == RfEncoding::PWM) {
        r.protocolGuess = "通用PWM编码";
        r.confidence = 0.4f;
    } else {
        r.protocolGuess = "未知协议";
        r.confidence = 0.2f;
    }
...
std::string SubGhzAnalyzeManager::analyzeFrequencyActivity(
    ...
    std::string modStr =
        (mod.first == ModGuess::ASK_OOK)   ? "ASK/OOK(幅移键控)" :
        (mod.first == ModGuess::FSK_GFSK) ? "FSK/GFSK(频移键控)" :
        (mod.first == ModGuess::NFM_FM)   ? "NFM/FM(窄带调频)"   : "未知调制";
...
std::string SubGhzAnalyzeManager::formatFrequency(
    ...
    oss << "峰值=" << peakDbm << " dBm"
        << "  活动率=" << static_cast<int>(hitRatio * 100.f) << '%'
        << "  置信度=" << static_cast<int>(confidence * 100.f) << '%'
        << "  平均=" << static_cast<int>(std::lround(avgDbm)) << " dBm"
        << " (波动=" << static_cast<int>(std::lround(stddevDbm)) << ")"
        << "  有效采样=" << hits << '/' << windows
        << "  调制=" << mod;
    return oss.str();
}
...
std::string SubGhzAnalyzeManager::formatFrame(const SubGhzDetectResult& r) const {
    auto encToStr = [](RfEncoding e){
        switch (e) {
            case RfEncoding::PulseLength: return "脉冲长度编码";
            case RfEncoding::Manchester:  return "曼彻斯特编码";
            case RfEncoding::PWM:         return "PWM编码";
            default:                      return "未知编码";
        }
    };
    ...
    std::ostringstream oss;
    oss << " [已接收帧]\r\n"
        << " 编码类型     : " << encToStr(r.encoding) << "\r\n"
        << " 基准周期(us) : " << std::lround(r.baseT_us) << "\r\n"
        << " 比特率(kb)   : " << r.bitrate_kbps << "\r\n"
        << " 比特总数     : " << r.bitCount << "\r\n"
        << " 数据载荷(hex): " << (r.payloadHex.empty() ? "-" : r.payloadHex) << "\r\n"
        << " 协议推测     : " << r.protocolGuess << "\r\n"
        << " 置信度       : " << r.confidence << "\r\n";
    if (!r.notes.empty()) oss << "备注         : " << r.notes << "\r\n";
    return oss.str();
}
