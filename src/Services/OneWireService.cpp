...
void OneWireService::writeRw1990(uint8_t pin, uint8_t* data, size_t len) {
    // 基于: https://github.com/ArminJo/iButtonProgrammer/blob/master/iButtonProgrammer.ino //汉化
    // @ArminJo
...
void OneWireService::configureEeprom(uint8_t pin) {
    ...
    owEeprom = new OneWireNg_CurrentPlatform(pin, false);  // parasitic=false (非寄生供电) //汉化
}
...
bool OneWireService::eeprom2431WriteRow(uint8_t rowAddr, const uint8_t* rowData, bool checkDataIntegrity) {
    ...
    // 读取暂存器 //汉化
    uint8_t rsp[1 + 2 + 1 + ROW_SIZE + 2]; // cmd + TA1/TA2 + ES + 数据 + CRC //汉化
...
bool OneWireService::getEepromModelInfos(uint8_t* romId, std::string& model, uint16_t& size, uint8_t& pageSize) {
    ...
    switch (familyCode) {
        case DS2431_FAMILY:
            model = "DS2431 (1 Kbit)"; // 型号名称保留原文 //汉化
            size = 128;
            pageSize = 8;
            return true;
        case DS2433_FAMILY:
